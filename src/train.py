...
def train():
  ...
  #exit(0)
  #indicates no data on start
  try:
    images, labels = inputs(FLAGS.batch, FLAGS.train, FLAGS.train_labels)
  except:
    print 'No data found at %s' % FLAGS.train
    exit(1)
  one_hot_labels = classifier.one_hot(labels)

  print '[train][values][one_hot_labels: %s, images: %s, labels: %s]' % (one_hot_labels.get_shape().as_list(), images.get_shape().as_list(), labels.get_shape().as_list())

  n_classification_classes = one_hot_labels.get_shape().as_list()[-1]
  autoencoder = SegNetAutoencoder(n_classification_classes)
  ...
  with tf.Session(config=config) as sess:
    ...
    summary_writer = tf.summary.FileWriter(FLAGS.train_logs, sess.graph) #tf.train.SummaryWriter(FLAGS.train_logs, sess.graph)
    ...
    for step in range(FLAGS.steps + 1):
      ...
      if step % 10 == 0 and coord.should_stop() == False:
